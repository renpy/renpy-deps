...
static int MPGLIB_open(Sound_Sample *sample, const char *ext)
{
    ...
    {
        Uint8 id3[10];
        if (SDL_RWread(internal->rw, id3, sizeof(id3), 1) != 1) {
            BAIL_MACRO("MPGLIB: Failure reading ID3 magic.", 0);
        }

            
        if (id3[0] == 'I' && id3[1] == 'D' && id3[2] == '3') {
            int id3len = id3[6] << 21 | id3[7] << 14 | id3[8] << 7 | id3[9];
            ...
            SDL_RWseek(internal->rw, id3len, SEEK_CUR);                
        } else {
            SDL_RWseek(internal->rw, -sizeof (id3), SEEK_CUR);                
        }
    }
    ...
    if (rc == MP3_ERR || rc == MP3_DONE_EARLY)
    {
        free(mpg);
        BAIL_MACRO("MPGLIB: Not an MP3 stream?", 0);
    } /* if */
